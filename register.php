...
if(isset($_POST['register'])){
   ...
   $email_check="SELECT * from `data` where email = '$user_email'";
   $email_query= mysqli_query($connect,$email_check);
   if(mysqli_num_rows($email_query)>0){
      echo "<script> alert ('email already exists')</script>";
   }
   else{
      $query="INSERT INTO `data` (`name`, `email`, `password`) VALUES ('$username', '$user_email', '$user_pass')";
      $result=mysqli_query($connect,$query);
      header('location:http://localhost:8082/techtemp-main/login.php');
   }
}
